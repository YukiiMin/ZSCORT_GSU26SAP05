...
*  --------------------------------------------------------------------*
*   SECTION 2: LOCAL CLASS - CONTROLLER
*  --------------------------------------------------------------------*
...
*  --------------------------------------------------------------------*
*   SECTION 5: LOCAL CLASS IMPLEMENTATIONS
*  --------------------------------------------------------------------*
CLASS lcl_controller IMPLEMENTATION.
  ...
  METHOD display_alv.
    ...
    DATA(lv_count_str) = CONV string( mv_total_count ).
    CONCATENATE 'SCORT - Repository Object Manager' lv_count_str 'hits'
  ...
  METHOD display_tr_alv.
    ...
    DATA(lv_count_str2) = CONV string( mv_total_count ).
    CONCATENATE 'TR Search Results' lv_count_str2 'objects found - DEV-032 SCORT'
      INTO lv_header SEPARATED BY ' '.
  ...
  METHOD on_user_command.
    ...
    CASE e_salv_function.
      ...
      WHEN 'GOTO_SE80'.
        ...
        IF ls_cell-row = 0.
          MESSAGE 'Please select a row first.' TYPE 'S' DISPLAY LIKE 'W'. RETURN.
        ENDIF.
        TRY.
            ls_sel = mt_objects[ ls_cell-row ].
          CATCH cx_sy_itab_line_not_found. RETURN.
        ENDTRY.
        CALL FUNCTION 'RS_TOOL_ACCESS'
          EXPORTING
            operation   = 'SHOW'
            object_name = ls_sel-obj_name
            object_type = ls_sel-object
            devclass    = ls_sel-devclass
          EXCEPTIONS
            not_executed = 1
            OTHERS       = 2.
        IF sy-subrc <> 0.
      ...
      WHEN 'CHANGE_PKG'.
        ...
        ls_cell = lo_selections->get_current_cell( ).
        IF ls_cell-row = 0.
          MESSAGE 'Please select an object row first.' TYPE 'S' DISPLAY LIKE 'W'. RETURN.
        ENDIF.
        TRY.
            ls_sel = mt_objects[ ls_cell-row ].
          CATCH cx_sy_itab_line_not_found. RETURN.
